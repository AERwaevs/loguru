...
Set to aer::log::internal::Verbosity_OFF to write nothing to stderr.
...
// Runtime options passed to aer::log::internal::init
struct Options
{
	...
	// aer::log::internal::init will set the name of the calling thread to this.
	// If you don't want logger to set the name of the main thread,
	// set this to nullptr.
	// NOTE: on SOME platforms aer::log::internal::init will only overwrite the thread name
	// if a thread name has not already been set.
	// To always set a thread name, use aer::log::internal::set_thread_name instead.
...
/*  Should be called from the main thread.
	...
	aer::log::internal::init() will look for arguments meant for the logger and remove them.
	Arguments meant for the logger are:
		-v n   Set aer::log::internal::g_stderr_verbosity level. Examples:
	...
	Tip: You can set g_stderr_verbosity before calling aer::log::internal::init.
	That way you can set the default but have the user override it with the -v flag.
	Note that -v does not affect file logging (see aer::log::internal::add_file).
...
// Returns all arguments given to aer::log::internal::init(), but escaped with a single space as separator.

const char* arguments();

// Returns the path to the current working dir when aer::log::internal::init() was called.
...
/*  Will log to a file at the given path.
	...
	If path starts with a ~, it will be replaced with aer::log::internal::home_dir()
	To stop the file logging, just call aer::log::internal::remove_callback(path) with the same path.
*/

bool add_file(const char* path, FileMode mode = FileMode::Truncate, Verbosity verbosity = Verbosity_INFO );
...
/* Generates a readable stacktrace as a string.
	...
	don't include the call to aer::log::internal::stacktrace in the stack trace. */
...
/*  Add a string to be replaced with something else in the stack output.
	...
	You can clean it up with:
		auto verbose_type_name = aer::log::internal::demangle(typeid(std::ofstream).name());
		aer::log::internal::add_stack_cleanup(verbose_type_name.c_str(); "std::ofstream");
...
Usage:
	printf("%sRed%sGreen%sBold green%sClear again\n",
			aer::log::internal::terminal_red(), aer::log::internal::terminal_green(),
			aer::log::internal::terminal_bold(), aer::log::internal::terminal_reset());
...
/* 	A stack trace gives you the names of the function at the point of a crash.
	...
	Use aer::log::internal::get_error_context() to get the contents of the active error contexts.
...
#define ERROR_CONTEXT(descr, data)                                             \
	const aer::log::internal::EcEntryData<aer::log::internal::make_ec_type<decltype(data)>::type>      \
		LOG_ANONYMOUS_VARIABLE(error_context_scope_)(                       \
			__FILE__, __LINE__, descr, data,                                   \
			static_cast<aer::log::internal::EcEntryData<aer::log::internal::make_ec_type<decltype(data)>::type>::Printer>(aer::log::internal::ec_to_text) ) // For better error messages

/*
#define ERROR_CONTEXT(descr, data)                                 \
	const auto LOG_ANONYMOUS_VARIABLE(error_context_scope_)(    \
		aer::log::internal::make_ec_entry_lambda(__FILE__, __LINE__, descr,    \
			[=](){ return aer::log::internal::ec_to_text(data); }))
*/
...
/*
	...
	You can pass the handle to aer::log::internal::get_error_context on another thread.
	...
	void foo(const char* parameter)
	{
		...
		const auto parent_ec_handle = aer::log::internal::get_thread_ec_handle();

		std::thread([=]{
			aer::log::internal::set_thread_name("child thread");
...
#define VLOG_F(verbosity, ...)                                                                     \
	((verbosity) > aer::log::internal::current_verbosity_cutoff()) ? (void)0                                   \
									  : aer::log::internal::log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

// LOG_F(INFO, "Foo: %d", some_number);
#define LOG_F(verbosity_name, ...) VLOG_F(aer::log::internal::Verbosity_ ## verbosity_name, __VA_ARGS__)

#define VLOG_IF_F(verbosity, cond, ...)                                                            \
	((verbosity) > aer::log::internal::current_verbosity_cutoff() || (cond) == false)                          \
		? (void)0                                                                                  \
		: aer::log::internal::log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define LOG_IF_F(verbosity_name, cond, ...)                                                        \
	VLOG_IF_F(aer::log::internal::Verbosity_ ## verbosity_name, cond, __VA_ARGS__)

#define VLOG_SCOPE_F(verbosity, ...)                                                               \
	aer::log::internal::LogScopeRAII LOG_ANONYMOUS_VARIABLE(error_context_RAII_) =                          \
	((verbosity) > aer::log::internal::current_verbosity_cutoff()) ? aer::log::internal::LogScopeRAII() :                  \
	aer::log::internal::LogScopeRAII(verbosity, __FILE__, __LINE__, __VA_ARGS__)

// Raw logging - no preamble, no indentation. Slightly faster than full logging.
#define RAW_VLOG_F(verbosity, ...)                                                                 \
	((verbosity) > aer::log::internal::current_verbosity_cutoff()) ? (void)0                                   \
									  : aer::log::internal::raw_log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define RAW_LOG_F(verbosity_name, ...) RAW_VLOG_F(aer::log::internal::Verbosity_ ## verbosity_name, __VA_ARGS__)

// Use to book-end a scope. Affects logging on all threads.
#define LOG_SCOPE_F(verbosity_name, ...)                                                           \
	VLOG_SCOPE_F(aer::log::internal::Verbosity_ ## verbosity_name, __VA_ARGS__)
...
// Message is optional
#define ABORT_F(...) aer::log::internal::log_and_abort(0, "ABORT: ", __FILE__, __LINE__, __VA_ARGS__)

#define ABORT_IF_F( cond, ...)                                                        \
	((cond) == false)                          \
		? (void)0                                                                                  \
		: aer::log::internal::log_and_abort(0, "ABORT: ", __FILE__, __LINE__, __VA_ARGS__)

// --------------------------------------------------------------------
// CHECK_F macros:

#define CHECK_WITH_INFO_F(test, info, ...)                                                         \
	LOG_PREDICT_TRUE((test) == true) ? (void)0 : aer::log::internal::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__,      \
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                 \
	...
	{                                                                                              \
		...
		{                                                                                          \
			auto str_left = aer::log::internal::format_value(val_left);                                        \
			auto str_right = aer::log::internal::format_value(val_right);                                      \
			auto fail_info = aer::log::internal::textprintf("CHECK FAILED:  %s %s %s  (%s %s %s)  ",           \
				#expr_left, #op, #expr_right, str_left.c_str(), #op, str_right.c_str());           \
			auto user_msg = aer::log::internal::textprintf(__VA_ARGS__);                                       \
			aer::log::internal::log_and_abort(0, fail_info.c_str(), __FILE__, __LINE__,                        \
			                      "%s", user_msg.c_str());                                           \
		}                                                                                          \
	} while (false)
...
#define VLOG_IF_S(verbosity, cond)                                                                 \
	((verbosity) > aer::log::internal::current_verbosity_cutoff() || (cond) == false)                          \
		? (void)0                                                                                  \
		: aer::log::internal::Voidify() & aer::log::internal::StreamLogger(verbosity, __FILE__, __LINE__)
#define LOG_IF_S(verbosity_name, cond) VLOG_IF_S(aer::log::internal::Verbosity_ ## verbosity_name, cond)
#define VLOG_S(verbosity)              VLOG_IF_S(verbosity, true)
#define LOG_S(verbosity_name)          VLOG_S(aer::log::internal::Verbosity_ ## verbosity_name)

// -----------------------------------------------
// ABORT_S macro. Usage:  ABORT_S() << "Causo of error: " << details;

#define ABORT_S() aer::log::internal::Voidify() & aer::log::internal::AbortLogger("ABORT: ", __FILE__, __LINE__)
...
#define CHECK_WITH_INFO_S(cond, info)                                                              \
	LOG_PREDICT_TRUE((cond) == true)                                                            \
		? (void)0                                                                                  \
		: aer::log::internal::Voidify() & aer::log::internal::AbortLogger("CHECK FAILED:  " info "  ", __FILE__, __LINE__)
...
#define CHECK_OP_S(function_name, expr1, op, expr2)                                                \
	while (auto error_string = aer::log::internal::function_name(#expr1 " " #op " " #expr2,                    \
													 aer::log::internal::referenceable_value(expr1), #op,      \
													 aer::log::internal::referenceable_value(expr2)))          \
		aer::log::internal::AbortLogger(error_string->c_str(), __FILE__, __LINE__)
...
#define CHECK_GT_S(expr1, expr2) CHECK_OP_S(check_GT_impl, expr1, > , expr2)
	...
	#define DVLOG_IF_S(verbosity, cond)                                                     \
		(true || (verbosity) > aer::log::internal::current_verbosity_cutoff() || (cond) == false)       \
			? (void)0                                                                       \
			: aer::log::internal::Voidify() & aer::log::internal::StreamLogger(verbosity, __FILE__, __LINE__)

	#define DLOG_IF_S(verbosity_name, cond) DVLOG_IF_S(aer::log::internal::Verbosity_ ## verbosity_name, cond)
	#define DVLOG_S(verbosity)              DVLOG_IF_S(verbosity, true)
	#define DLOG_S(verbosity_name)          DVLOG_S(aer::log::internal::Verbosity_ ## verbosity_name)
#endif
